...
export function requestUrl(siteurl){
  ...
  return phantom.create(['--ignore-ssl-errors=yes'])
    ...
    .then(page => {
      sitepage = page;
      sitepage.property('onResourceReceived') = response => {
        if (response.stage !== "end"){ return; }
        resources.push(response)
      }
      return sitepage;
    }).then(() => {
      sitepage.open(siteurl);
      console.log(`Requesting ${siteurl}`)
    }).then(status => {
        if (status !== "success"){
          throw `HTTP Request error: ${status}`
        }
    }).then(() => sitecontent = sitepage.property('content'))
    .then(() => {
        sitepage.close(); // TODO: return sitepage to allow screenshotting
        return {content: sitecontent, response = collectedResources};
    });
